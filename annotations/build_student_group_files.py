...
# 	output.to_csv("newStudentAnnotations.csv", sep='\t')
# 	print(output.head())
# 	print(output.shape)
...
def get_student_files(studentData):
	...
	for index, row in studentData.iterrows():
		...
		elif counter_rows >= 100:
			if counter_rows < 300 and student_counter == 1:
				row_dict1["parent_id"].append(row.parent_id)
				row_dict1["child_id"].append(row.child_id)
				row_dict1["subreddit"].append(row.subreddit)
				row_dict1["parent_text"].append(row.parent_text)
				...
				if counter_rows == 300:
					student_counter += 1
			elif counter_rows >= 300 and counter_rows < 500 and student_counter == 2:
				...
				if counter_rows == 500:
					student_counter += 1
			elif counter_rows >= 500 and counter_rows < 700 and student_counter == 3:
				...
				if counter_rows == 700:
					student_counter += 1
			elif counter_rows >= 700 and counter_rows < 900 and student_counter == 4:
...
studentData = pd.read_csv('newStudentAnnotations.csv', sep="\t")

#get_parents(reddit, data)

# Split up the data in 5 x 4 x 300 (100 same posts per group, everybody 200 individual)
